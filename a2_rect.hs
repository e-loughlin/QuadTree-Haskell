...
--quadTreeHTML x y (RectImage w h qt)
--   | (w > h) = (quadTreeHTML x y (SquareImage w qt))
--   | otherwise = (quadTreeHTML x y (SquareImage h qt))
quadTreeHTML x y (RectImage w h qt)
   | (w > h) = (quadTreeHTML x y (SquareImage h qt)) ++ (quadTreeHTML (x + h) y (RectImage (w-h) h qt))
   | otherwise = (quadTreeHTML x y (SquareImage w qt)) ++ (quadTreeHTML x (y+w) (RectImage w (h-w) qt))
